...
def avg_medal_count():
    ...
    # YOUR CODE HERE
    xf = df[df.gold > 0].bronze
    xdf = DataFrame(xf);
    print xdf.apply(numpy.mean)
    avg_bronze_at_least_one_gold = xdf.apply(numpy.mean).bronze
    # avg_bronze_at_least_one_gold= df.silver[df.iloc(df[gold]>0)]
